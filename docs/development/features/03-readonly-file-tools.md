...
- Workspace + `filesystem.read` PermissionGrant 与规范绝对路径；
- 防止 `..`、符号链接、junction、UNC 和大小写逃逸；
- 文件大小、目录深度、搜索结果和编码限制；
- ToolResult 大内容外置引用；
- 每个调用进入 Operation Ledger，但 `side_effect=none`；
- 只读工具也必须经过完整 Policy Engine；只读表示无副作用，不表示可以绕过 principal、workspace scope、审计或预算；
- 文件在读取过程变化时返回版本信息。
...
- 路径攻击测试通过；
- GUI 可以显示本次授权的根目录。
- WorkspaceGrant 撤销后，后续读取立即失败且 Policy cache 不复用旧 revision。
